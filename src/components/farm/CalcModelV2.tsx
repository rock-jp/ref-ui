...
export default function CalcModelV2(
  props: ReactModal.Props & {
    originFarms: FarmKind[] & extendType;
    tokenPriceList: Record<string, string>;
  }
) {
  const { originFarms, tokenPriceList } = props;
  ...
  const farms = mergeCommonRewardFarmsFun(originFarms);
  const pool = originFarms[0].pool;
  ...
  function mergeCommonRewardFarmsFun(farmsData: FarmKind[] & extendType) {
    ...
    Object.keys(tempMap).forEach((m: string) => {
      const commonRewardArr: FarmKind[] & extendType = tempMap[m];
      if (commonRewardArr.length > 1) {
        ...
        for (let i = 1; i < commonRewardArr.length; i++) {
          ...
          target.rewardsPerWeekAmount = BigNumber.sum(
            ...
            commonReward.rewardsPerWeekAmount
          ).valueOf();
          target.userUnclaimedRewardAmount = BigNumber.sum(
            target.userUnclaimedRewardAmount,
            commonReward.userUnclaimedRewardAmount
          ).valueOf();
        }
        tempMap[m] = [target];
      }
